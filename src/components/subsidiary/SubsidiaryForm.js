...
const SubsidiaryForm = ({ openDialog, setOpenDialog }) => {
  ...
  const [createSubsidiary] = useMutation(CREATE_SUBSIDIARY, { 
    update: (cache, { data: { createSubsidiary  }}) => {
      if (createSubsidiary) {
        const data = cache.readQuery({ query: SUBSIDIARIES_OF_CURRENT_COMPANY }); 
        const newSubsidiary = createSubsidiary.subsidiary;
        cache.writeQuery({
          query: SUBSIDIARIES_OF_CURRENT_COMPANY,
          data: {
            subsidiariesOfCurrentCompany: [...data.subsidiariesOfCurrentCompany,newSubsidiary],
          }
        })
      }
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <DialogContent>
          <PlacesAutocomplete
            ...
            searchOptions={{ componentRestrictions: { country: ['chl'] } }}
          >
            {({
              getInputProps,
              suggestions,
